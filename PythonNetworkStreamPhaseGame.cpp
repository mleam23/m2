...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_BIOLOGIST_SET_INFO:
				ret = RecvBiologistSetInfo();
				break;
#endif
...
bool CPythonNetworkStream::RecvGrowthPetInformation()
{
	...
	if (pack.dwSealItemVnum == 0) {
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_GrowthPetSetInfo", Py_BuildValue("(iisiiiiiiiiiiiiiiiiiiii)", 
...
bool CPythonNetworkStream::SendPrivateShopSearchBuyPacket(WORD wPos)
{
	...
	subPacket.TPrice.dwCheque = c_pItemData->TPrice.dwCheque;
...
#ifdef ENABLE_BIOLOGIST_SYSTEM
bool CPythonNetworkStream::SendBiologistAction(BYTE bAction, int iValue0, int iValue1, int iValue2)
{
	if (!__CanActMainInstance())
		return true;

	TPacketGCBiologistAction packet;
	packet.bHeader = HEADER_CG_SEND_BIOLOGIST_ACTION;
	packet.bAction = bAction;
	packet.iValue0 = iValue0;
	packet.iValue1 = iValue1;
	packet.iValue2 = iValue2;

	if (!Send(sizeof(TPacketGCBiologistAction), &packet))
		return false;

	return SendSequence();
}

bool CPythonNetworkStream::RecvBiologistSetInfo()
{
	TPacketGCBiologistInfo packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	CPythonPlayer::Instance().ClearBiologistInfoVector();
	for (int i = 0; i < BIOLOGIST_MISSION_MAX_NUM; ++i)
	{
		TPlayerBiologist biologistInfo;
		biologistInfo.bMissionState = packet.biologistInfo[i].bMissionState;
		biologistInfo.dwCollectCount = packet.biologistInfo[i].dwCollectCount;
		biologistInfo.dwHasBonusChance = packet.biologistInfo[i].bMissionState;
		biologistInfo.dwWaitingStart = packet.biologistInfo[i].dwWaitingStart;
		biologistInfo.dwWaitingEnd = packet.biologistInfo[i].dwWaitingEnd;
		biologistInfo.dwWaitingBonus = packet.biologistInfo[i].dwWaitingBonus;
		biologistInfo.bSelectedBonus = packet.biologistInfo[i].bSelectedBonus;

		CPythonPlayer::Instance().SetBiologistInfoData(i, biologistInfo);
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_BiologistOpenWindow", Py_BuildValue("(ii)", packet.dLevel, packet.dTarget));
